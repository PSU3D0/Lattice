...
use llm_types::completion::{CompletionError, CompletionRequest, GetTokenUsage};
use llm_types::http_client::sse::{Event, GenericEventSource};
use llm_types::http_client::{self, HttpClientExt};
use llm_types::json_utils::merge_inplace;
use llm_types::message::ReasoningContent;
use llm_types::streaming::{
    ...
    T: HttpClientExt + Clone + Default + 'static,
{
    ...
    ) -> Result<streaming::StreamingCompletionResponse<StreamingCompletionResponse>, CompletionError>
    {
        ...
        let _span = if tracing::Span::current().is_disabled() {
            info_span!(
                                ...
                                match &event {
                                    ...
                                    StreamingEvent::MessageDelta { delta, usage } => {
                                        if delta.stop_reason.is_some() {
                                            ...
                                            let span = tracing::Span::current();
                                            if !span.is_disabled() {
                                                span.record("gen_ai.usage.input_tokens", usage.input_tokens.unwrap_or(0) as u64);
    ...
    match event {
        ...
        StreamingEvent::ContentBlockStart { content_block, .. } => match content_block {
            Content::ToolUse { id, name, .. } => {
                let internal_call_id = generate_call_id();
...
mod tests {
    ...
    fn test_thinking_delta_does_not_interfere_with_tool_calls() {
        ...
        let mut tool_call_state = Some(ToolCallState {
            ...
            internal_call_id: generate_call_id(),
            input_json: String::new(),
        });
        let mut thinking_state = None;
