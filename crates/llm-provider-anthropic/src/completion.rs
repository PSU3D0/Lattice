...
use llm_types::completion::CompletionRequest;
...
fn to_one_or_many<T: Clone>(items: Vec<T>, error: &'static str) -> Result<OneOrMany<T>, MessageError> {
    OneOrMany::many(items).map_err(|_| MessageError::ConversionError(error.to_owned()))
}
        ...
        Ok(match message {
            message::Message::User { content } => {
                    ...
                    .map(|content| match content {
                        ...
                        message::UserContent::ToolResult(message::ToolResult { id, content, .. }) => {
                            ...
                            Ok(Content::ToolResult {
                                tool_use_id: id,
                                content: to_one_or_many(
                                    content,
                                    "Tool result content did not contain any content",
                                )?,
                                is_error: None,
                                cache_control: None,
                            })
                ...
                Message {
                    ...
                    content: to_one_or_many(
                        content,
                        "User message did not contain any content",
                    )?,
                }
            }
        ...
        Ok(match message.role {
            Role::User => message::Message::User {
                content: {
                    ...
                    to_one_or_many(content, "User message did not contain any content")?
                },
            },
            ...
            if response.status().is_success() {
                match serde_json::from_slice::<ApiResponse<CompletionResponse>>(
                    ...
                    ApiResponse::Message(completion) => {
                        ...
                        if !span.is_disabled() {
                            span.record("gen_ai.response.id", &completion.id);
                            span.record("gen_ai.response.model", &completion.model);
                            ...
                            span.record(
                                ...
                                completion.usage.cache_creation_input_tokens.unwrap_or(0),
                            );
                        }
                        if enabled!(Level::TRACE) {
                            tracing::trace!(
...
mod tests {
    ...
    fn test_content_format_conversion() {
        use llm_types::completion::message::ContentFormat;
    ...
    fn test_plaintext_rig_to_anthropic_conversion() {
        use llm_types::completion::message as msg;
    ...
    fn test_assistant_reasoning_multiblock_to_anthropic_content() {
        let mut reasoning = message::Reasoning::new("step one");
    ...
    fn test_assistant_encrypted_reasoning_maps_to_redacted_thinking() {
        let mut reasoning = message::Reasoning::new("ignored");
        reasoning.content = vec![message::ReasoningContent::Encrypted(
