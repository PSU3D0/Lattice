...
use super::{
    client::{ApiErrorResponse, ApiResponse, Client},
    streaming::StreamingCompletionResponse,
};
use llm_types::completion::{
    CompletionError, CompletionRequest as CoreCompletionRequest, GetTokenUsage,
};
use llm_types::http_client::{self, HttpClientExt};
use llm_types::message::{AudioMediaType, DocumentSourceKind, ImageDetail, MimeType};
use llm_types::one_or_many::string_or_one_or_many;
use llm_types::wasm_compat::{WasmCompatSend, WasmCompatSync};
use llm_types::{OneOrMany, completion, json_utils, message};
...
impl TryFrom<llm_types::message::ToolChoice> for ToolChoice {
    type Error = CompletionError;
    fn try_from(value: llm_types::message::ToolChoice) -> Result<Self, Self::Error> {
        let res = match value {
...
fn user_content_to_openai_messages(
...
fn assistant_content_to_openai_messages(
...
fn core_message_to_openai_messages(
    message: message::Message,
) -> Result<Vec<Message>, message::MessageError> {
    match message {
        message::Message::System { content } => Ok(vec![Message::system(&content)]),
        message::Message::User { content } => user_content_to_openai_messages(content),
        message::Message::Assistant { content, .. } => assistant_content_to_openai_messages(content),
    }
}
    ...
    fn try_from(message: Message) -> Result<Self, Self::Error> {
        Ok(match message {
            Message::User { content, .. } => message::Message::User {
                content: OneOrMany::many(content.into_iter().map(|content| content.into()).collect::<Vec<_>>())
                    .map_err(|_| {
                        message::MessageError::ConversionError(
...
impl TryFrom<OpenAIRequestParams> for CompletionRequest {
    ...
    fn try_from(params: OpenAIRequestParams) -> Result<Self, Self::Error> {
        ...
        full_history.extend(
            partial_history
                .into_iter()
                .map(core_message_to_openai_messages)
...
{
    pub fn into_agent_builder(self) -> llm_agent::agent::AgentBuilder<Self> {
        llm_agent::agent::AgentBuilder::new(self)
    }
}
        ...
        + 'static,
{
    ...
    async fn stream(
        ...
        llm_types::streaming::StreamingCompletionResponse<Self::StreamingResponse>,
        CompletionError,
    > {
...
mod tests {
    ...
    fn test_openai_request_uses_request_model_override() {
        let request = llm_types::completion::CompletionRequest {
            model: Some("gpt-4.1".to_string()),
            preamble: None,
            chat_history: llm_types::OneOrMany::one("Hello".into()),
            documents: vec![],
            tools: vec![],
            temperature: None,
            max_tokens: None,
    ...
    fn assistant_reasoning_is_silently_skipped() {
        ...
        let converted: Vec<Message> = assistant_content_to_openai_messages(assistant_content)
