...
use example_s14_meeting_transcript_sync::cloudflare::D1TranscriptJobStore;
...
async fn main() -> anyhow::Result<()> {
    ...
    let http =
        Arc::new(ReqwestHttpClient::with_default_tls().context("build reqwest HTTP capability")?);
    ...
    let store = D1TranscriptJobStore::open_path(&local.ledger_path)
        .with_context(|| format!("open local D1-shaped ledger at {}", local.ledger_path))?;
