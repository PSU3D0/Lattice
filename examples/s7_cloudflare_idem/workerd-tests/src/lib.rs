...
use example_s7_cloudflare_idem::{
    bundle, clear_worker_context, config_from_env, set_worker_context,
};
...
async fn fetch(req: Request, env: Env, ctx: Context) -> Result<Response> {
    ...
    let dedupe = WorkersDurableObject::from_binding(binding.clone(), Some("preflight".to_string()));
    let durability = Arc::new(WorkersDurableObject::from_binding(
        binding,
        Some("durability-checkpoints".to_string()),
    ));
